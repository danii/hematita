    ...
    T: Iterator<Item = char>,
{
        ...
        loop {
            match self.peek()? {
                ...
                character if character == delimiter => {
                    self.eat();
                    break Some(Ok(Token::String(string)));
                }
    ...
    fn parse_number(&mut self) -> Result<Token> {
        ...
        if is_float {
            return number
                .parse::<f64>()
                .map(|float| Token::Double(Double(float)))
            ...
            _ => {
                ...
                loop {
                    match self.peek()? {
                        '\n' => {
                            self.eat();
                            break Some(Token::Comment(comment));
                        }
    ...
    T: Iterator<Item = char>,
{
    ...
    fn next(&mut self) -> Option<Result<Token>> {
        match self.parse_whitespace()? {
            ...
            '^' => {
                self.eat();
                Some(Ok(Token::Exponent))
            }

            // Bitwise
            '&' => {
                self.eat();
                Some(Ok(Token::BitwiseAnd))
            }
            '|' => {
                self.eat();
                Some(Ok(Token::BitwiseOr))
            }
